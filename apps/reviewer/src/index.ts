...
async function enqueueRebaseRevise(
  ...
  await store.insertTask({
    ...
    body: [
      ...
      `Resolve them on the SAME branch: merge the latest \`${pr.baseRefName}\` into \`${pr.headRefName}\`, fix the conflicts, run verify, and push. Do NOT open a new PR.`,
      "",
      "### Conflict-resolution playbook — resolve SEMANTICALLY, not just textually",
      "- **DB migrations** (`db/migrations/*`): if your migration's prefix now clashes with one that landed on the base, RENAME your file to a fresh timestamp prefix so it sorts AFTER the base's — never reuse or duplicate a prefix, and never delete the base's. Keep your SQL; only the filename prefix changes.",
      "- **Lockfiles** (`pnpm-lock.yaml` / `package-lock.json`): do NOT hand-merge. Take the base's version, then re-run the repo's install (`pnpm install`) to regenerate against the merged `package.json`.",
      "- **`package.json`**: keep the UNION of both sides' dependencies/scripts; only pick a side when the same key genuinely diverges.",
      "- **Generated / imported code**: keep both sides' additions and regenerate anything derived.",
      "- If a conflict is a real logic divergence you cannot safely reconcile, STOP and leave a comment explaining it rather than guessing.",
    ].join("\n"),
    prNumber: pr.number,
