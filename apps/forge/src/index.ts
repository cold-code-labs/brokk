...
async function main() {
  ...
  const lane =
    wantEngine === "cursor-cli" && cursorCliAvailable()
      ? "cursor-cli"
      : wantEngine === "openhands" && openHandsCliAvailable()
        ? "openhands"
  ...
  if (wantEngine === "openhands" && lane !== "openhands") {
    console.error(
      "[forge] BROKK_FORGE_ENGINE=openhands but openhands binary/LLM_BASE_URL+LLM_API_KEY missing — falling back to the native engine",
    );
  }
  ...
  const engine =
    lane === "cursor-cli"
      ...
      : lane === "openhands"
        ? new OpenHandsCliEngine({ browser: cfg.browser })
        : lane === "cli"
  ...
  const LANE_LABEL = {
    ...
    openhands: "openhands (LiteLLM→OmniRoute)",
    cli: "cli (Claude Code)",
    afl: "afl (native)",
  } as const;
...
async function runDevLane(
  ...
  try {
    ...
    // Heimdall hands back — NOT the retired HAULDR_TOKEN data-plane management
    // key. That key 401s the migrate endpoint and leaves the agent with a silent
    // dead tool (BROKK-14). Same project + endpoint the deploy migrates through
    // → file and live schema stay identical.
    ...
    if (cfg.hauldrControlUrl && lanes) {
      try {
        ...
        if (!hp.migrateToken) {
          console.error(
            `[forge] apply_migration DISABLED for ${project}: Heimdall returned no migrateToken`,
          );
        } else {
          migration = { controlUrl: cfg.hauldrControlUrl, token: hp.migrateToken, project };
        }
      } catch (err) {
        console.error(
