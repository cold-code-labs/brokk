...
/**
 * GitHub App auth for chat/sindri's git operations (clone/fetch/worktree). We sign
 * a short-lived JWT with the Eitri App's private key and exchange it for an
 * installation access token (valid 1h, cached), used as GH_TOKEN so the
 * `gh auth git-credential` helper authenticates git over HTTPS. Durable by design:
 * the App private key doesn't expire like a classic PAT, so a stale PAT never
 * strands session checkouts / QA-discovery scouts again (the reason this was wired
 * in — a dead GITHUB_TOKEN silently froze `ensure` on a month-old cached tree).
 * Mirrors apps/forge/src/github-app.ts and apps/reviewer/src/github-app.ts.
 */
