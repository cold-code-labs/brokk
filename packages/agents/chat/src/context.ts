...
const IDENTITY = [
  ...
  "- CLOSE THE LOOP: editing files is NOT finishing. When the user wants to SEE a change, you are done ONLY after you commit and PUSH it so the live preview updates — the preview serves this repo's `dev` branch, not your session's working branch, so an unpushed edit never shows. Follow the repo's CLAUDE.md/AGENTS.md publishing convention (typically `git push origin HEAD:dev`); run typecheck first. Never leave a wanted change committed-but-unpushed or staged; state plainly whether you published.",
