...
4. **MVP:** `openhands --headless --json --override-with-envs -t …` no worktree Brokk. Agent-server / Docker sandbox OH = fase posterior.
5. **Default da frota** permanece `cursor-cli` até shadow worker verde; cutover por env/flag.
...
- Imagem forge opcionalmente instala CLI `openhands`; sem binário → fallback ruidoso para `afl`/`cursor-cli`.
- Observabilidade: eventos JSONL OH → `AgentEvent` (board/Langfuse).
- Próximo: Sindri `ChatEngine=openhands`; agent-server para resume longo.
